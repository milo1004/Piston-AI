...
def downloadSTTModel(url:str="https://alphacephei.com/vosk/models/vosk-model-en-us-0.22-lgraph.zip"):
...
def downloadWakeModel(url:str="https://alphacephei.com/vosk/models/vosk-model-small-en-us-0.15.zip"):
    response = requests.get(url)
    response.raise_for_status()

    with open("WakeVOSK.zip","wb") as f:
        try:
            f.write(response.content)
            return True
        except Exception as e:
